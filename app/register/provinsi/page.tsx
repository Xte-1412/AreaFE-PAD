...
export default function RegisterProvinsiPage() {
  ...
  const [isLoadingDinas, setIsLoadingDinas] = useState(true);

  // Fetch dinas provinsi
  useEffect(() => {
    setIsLoadingDinas(true);
    axios.get('/api/register/dinas/provinsi')
      .then(res => {
        setDinasList(res.data.data || []);
      })
      .catch(err => {
        console.error("Gagal mengambil dinas provinsi:", err);
        setFormError("Gagal memuat daftar DLH Provinsi.");
      })
      .finally(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!selectedDinas) {
      setFormError("Silakan pilih DLH Provinsi.");
      return;
    }
  ...
  return (
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {/* Pilih DLH */}
          <div>
            <label htmlFor="dinas" className="block text-left text-sm font-medium text-gray-700">Pilih DLH Provinsi</label>
            <select
              id="dinas"
              value={selectedDinas}
              onChange={(e) => setSelectedDinas(e.target.value)}
              required
              disabled={isLoadingDinas}
            ...
            >
              <option value="" disabled>
                {isLoadingDinas ? "Memuat..." : "-- Pilih DLH Provinsi --"}
              </option>
              {dinasList.map(dinas => (
          ...
          <div className="pt-4">
            <button
              type="submit"
              disabled={isLoading || !selectedDinas}
